    ...
    public class WindowManager : IDisposable
    {
        ...
        private void ApplyManualLayout(List<Taskbar> taskbars, bool completeDeferredLayout)
        {
            ...
            foreach (AppBarEdge edge in Settings.Instance.ResolvedEdgeOrder)
            {
                ...
                int thickness = taskbar.DesiredThicknessPixels;
                NativeMethods.Rect taskbarRect = workArea;

                switch (edge)
                {
                    case AppBarEdge.Left:
                        taskbarRect.Right = taskbarRect.Left + thickness;
                        workArea.Left += thickness;
                        break;
                    case AppBarEdge.Right:
                        taskbarRect.Left = taskbarRect.Right - thickness;
                        workArea.Right -= thickness;
                        break;
                    case AppBarEdge.Top:
                        taskbarRect.Bottom = taskbarRect.Top + thickness;
                        workArea.Top += thickness;
                        break;
                    case AppBarEdge.Bottom:
                        taskbarRect.Top = taskbarRect.Bottom - thickness;
                        workArea.Bottom -= thickness;
                        break;
                }
            ...
            ShellLogger.Debug($"WindowManager: Applied manual work area {FormatRect(workArea)}");
        }
