...
namespace UltraWinBar
{
    ...
    public partial class App : Application
    {
        ...
        private void App_OnStartup(object sender, StartupEventArgs e)
        {
            ...
            Settings.Instance.ShowQuickLaunch = false;
            loadTheme();
            _virtualDesktops = new VirtualDesktopContext();
            if (_virtualDesktops.CurrentId != Guid.Empty && Settings.Instance.PinnedApplications.Any(p => p.DesktopId == Guid.Empty))
            {
                var pins = Settings.Instance.PinnedApplications;
                foreach (var pin in pins.Where(p => p.DesktopId == Guid.Empty)) pin.DesktopId = _virtualDesktops.CurrentId;
                Settings.Instance.PinnedApplications = pins.ToList();
            }
        ...
        private void App_DispatcherUnhandledException(object sender, System.Windows.Threading.DispatcherUnhandledExceptionEventArgs e)
        {
            ...
            try
            {
                if (!_errorVisible)
                {
                    ...
                    {
                        if (MessageBox.Show(dMsg, "UltraWinBar Error", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes)
                        {
